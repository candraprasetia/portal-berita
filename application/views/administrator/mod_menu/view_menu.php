    ...
    <div class="box-body">
      ...
      <?php 
        ...
        foreach ($record as $row) {
          // Ambil data menu induk (parent)
          if ($row['id_parent'] == 0) {
            // Jika id_parent adalah 0, berarti ini Menu Utama
            $menu = 'Menu Utama';
          } else {
            // Cek jika ada parent menu, ambil datanya dari tabel menu
            $cmenu = $this->model_app->view_where('menu', array('id_menu' => $row['id_parent']))->row_array();
            if ($cmenu && isset($cmenu['nama_menu'])) {
              $menu = $cmenu['nama_menu'];
            } else {
              $menu = 'Menu Tidak Ditemukan'; // Jika menu induk tidak ditemukan
            }
          }
